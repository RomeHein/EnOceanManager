...
def learn(destination):
    global communicator
    communicator.send(
        RadioPacket.create(rorg=RORG.VLD, rorg_func=0x01, rorg_type=0x0C, destination=destination, sender=communicator.base_id, learn=True, command=1, IO=0x1E, OV=100)
    )
...
# learn([0x05, 0x9E, 0x25, 0xC7])
turn_on([0x05, 0x9E, 0x25, 0xC7])
# Needs a bit of sleep in between, working too fast :S
time.sleep(0.5)
# turn_on([0x01, 0x94, 0xE3, 0xB9])
# time.sleep(1)

turn_off([0x05, 0x9E, 0x25, 0xC7])
# time.sleep(0.1)
